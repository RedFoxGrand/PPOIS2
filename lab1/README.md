...
- Использует модуль json для сериализации объектов.
- **Особенность реализации:** Используется атомарная запись. Данные сначала пишутся во временный файл (.tmp), и только после успешной записи файл переименовывается в основной (university.json). Это предотвращает потерю данных при сбоях.
